...
var git = module.exports = {
	...
	pull: function(target, callback) {
		exec('git reset --hard HEAD && ' + 'git pull origin ' + shellescape([git.branch]), function(error, stdout, stderr) {
			if (error) {
	  			out = "" + error;
	...
	clone: function(target, callback) {
		...
		console.log(out.white);
		var gitclone = spawn('git', ['clone', shellescape([target.url]), shellescape([target.name])])
