...
db.open(function (error) {
    ...
    db.collection('jobs', function (err, collection) {
        if (err) {
            console.log("Error getting job collection");
            console.log(err);
        } else {
            console.log("Creating indexes".gray);
            collection.ensureIndex({_id:1}, function (err, i) {
                collection.ensureIndex({running:1, finished:1}, function (err, n) {
                    collection.ensureIndex({project:1, branch:1, addedTime:1}, function (err, m) {
                        collection.ensureIndex({project:1, branch:1, finished:1, failed:1, finishedTime:1}, function (err, l) {
                            console.log("Indexes created".gray);
                            return;
                        });
                    });
                });
...
getAllProjects = function (next) {
    return db.collection('jobs', function (error, collection) {
        ...
        if (collection) {
            collection.distinct("project", function (err, results) {
                if (err) {
