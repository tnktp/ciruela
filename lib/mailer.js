    ...
    sendBuildResult: function (buildResult) {
        ...
        emailTemplates(templatesDir, function(err, template) {
            ...
            template(config.emailTemplateDir, locals, function(err, html, text) {
                if (err) console.log(err.red);
                var subject = buildResult.testStatus + ': ' + buildResult.organization + '/' + buildResult.repoName +' [' + 
                              buildResult.branch + '] by ' + buildResult.commit.author.username + ': ' + buildResult.commit.message;
